...
def transcribe_audio(input_path, model_size, language="pt"):
    ...
    print(
        f"Transcribing in '{language}'... This may take some time depending on the length of the audio."
    )
...
def main():
    ...
    parser.add_argument(
        "file_path", help="Path to the input audio or video file or directory."
    )
    parser.add_argument(
        "--output-format",
        choices=["srt", "txt"],
    ...
    parser.add_argument(
        ...
        default="small",
        help="Whisper model size. Larger models may improve accuracy but take longer to process. 'small' is the default and recommended for a balance between speed and accuracy.",
    )
    parser.add_argument(
        "--language",
        default="pt",
        help="Language code for the transcription. Default is 'pt' for Portuguese.",
    )
    ...
    if os.path.isdir(args.file_path):
        transcribe_directory(
            args.file_path, args.model, args.output_format, args.language
        )
